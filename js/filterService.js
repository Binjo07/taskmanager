export function applyFilterAndSort (tasks, {priority, alpha, date, status, title}){
    ...
    if (status === 'completed'){
        processsedTasks = processsedTasks.filter(task=> task.completed === true)
    }else if (status === 'active'){
        processsedTasks = processsedTasks.filter(task => task.completed === false)
    }
    ...
    if (title){
        processsedTasks = processsedTasks.filter(task=>task.title.toLowerCase().includes(title))
    }
